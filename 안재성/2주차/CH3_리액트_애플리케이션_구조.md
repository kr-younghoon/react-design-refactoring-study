...
#### :bulb: 아토믹 디자인은 실무에서 유용할까?

- ProductCard 컴포넌트는 Card와 Button을 사용하니 원자는 아니지만 분자? 비즈니스 로직이 포함되어 있고 복잡하니 유기체?
- 원자, 분자, 유기체 라는 분류만으로 늘어나는 컴포넌트를 감당할 수 있을까?

-> 아토믹 구조는 비즈니스에서 개념적 영역인 도메인을 제대로 커버하지 못한다는 문제

#### 컴포넌트는 최대한 재사용이 가능하게 작성하는것이 원칙인데?

button 컴포넌트와 같은 순수 UI 컴포넌트에는 이러한 접근법이 용이하다.
...
하지만 큰 틀만 공유하고 내부 구조가 달라지는 여러 경우에 하나의 래퍼 컴포넌트로 대처하려하다 보면 props가 늘어나는걸 막을 수 없다.
...
이러한 경우에는 전통적인 방식으로 재사용이 잘 이루어지지 않는다. 하나의 컴포넌트로 억지로 묶으려다 보니 오히려 이상한 방향으로 진행된다.

이러한 시행착오 이후 중요해진 개념으로 순수 UI 컴포넌트와 도메인 컴포넌트의 분리가 있다.
...
사용자 요구사항은 도메인을 중심으로 발전하기 때문에 도메인을 다루는 컴포넌트와 UI를 다루는 컴포넌트는 서로 다른 관점에서 접근하고 분리해서 관리하는 방향으로 진화
